app.controller('mainCtrl', function($scope, $route, $routeParams, $rootScope, $location, $timeout, $_global, cons) {
    ...
    if ($rootScope.online) {
        ...
        $scope.scadaHost.client.addMessage = function(message) {
            ...
            if (msg.TagName == "rf_log" || msg.TagName == "rf_real") {
                var tag = 'ปริมาณฝน';
            }
            if (msg.TagName == "wl1_real" || msg.TagName == "wl1_log") {
                var tag = 'ระดับน้ำ(หน้า)';
            }
            if (msg.TagName == "wl2_real" || msg.TagName == "wl2_log") {
                var tag = 'ระดับน้ำ(ท้าย)';
            }
            if (["rf_log", "rf_real", "wl1_real", "wl1_log", "wl2_real", "wl2_log"].indexOf(msg.TagName) > -1) {
                var obj = {
                    id: msg.StationID,
                    code: selected.code,
                    name: selected.name,
                    ...
                    tag: tag,
                    param: msg.TagName,
                    value: parseFloat(msg.TagValue).toFixed(2) / 1
                };
            ...
            if (keepGoing) {
                if (data.id == value['id']) {
                    if (data.param == "rf_log" || data.param == "rf_real") {
						...
						$scope.dataREAL[key].rf.status = (data.value >= $scope.dataSTN[key]['data']['rf']['value']['danger']) ? "danger" : (data.value >= $scope.dataSTN[key]['data']['rf']['value']['warning']) ? "warning" : "success";
                    } else if (data.param == "wl1_real" || data.param == "wl1_log") {
                        $scope.dataREAL[key].wl_up.value = data.value;
                        $scope.dataREAL[key].date = data.date;
						$scope.dataREAL[key].wl_up.status = (data.value >= $scope.dataSTN[key]['data']['wl_up']['value']['danger']) ? "danger" : (data.value >= $scope.dataSTN[key]['data']['wl_up']['value']['warning']) ? "warning" : "success";
                    } else if (data.param == "wl2_real" || data.param == "wl2_log") {
                        $scope.dataREAL[key].wl_down.value = data.value;
                        $scope.dataREAL[key].date = data.date;
						$scope.dataREAL[key].wl_down.status = (data.value >= $scope.dataSTN[key]['data']['wl_down']['value']['danger']) ? "danger" : (data.value >= $scope.dataSTN[key]['data']['wl_down']['value']['warning']) ? "warning" : "success";
                    }
                    keepGoing = false;
                }
            }
...
app.controller('popCtrl', function($scope, $location, $timeout, $_global) {
    ...
    scope.getID = function(id) {
        ...
        scope.timeout = $scope.convIcon(scope.selected.timeout, '');
        scope.door = $scope.convIcon(scope.selected.door, 'door');
    };
    $timeout(function() {
...
app.controller('cfgCtrl', function($scope, $route, $timeout, $_global) {
    var scope = this;
    $timeout(function() {
